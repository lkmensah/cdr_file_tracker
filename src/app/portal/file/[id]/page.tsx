...
export default function PortalFileDetail() {
    ...
    const { attorney, isSG, isActingSG } = usePortal();
    ...
    const attorneyOptions = React.useMemo(() => {
        return (allAttorneys || []).map(a => ({ label: a.fullName, value: a.id }));
    }, [allAttorneys]);
    ...
    const isInMyGroup = ( (attorney?.isGroupHead || attorney?.isActingGroupHead) && !!myGroup && myGroup !== 'no group yet' && fileGroup === myGroup) || isSG;
    ...
    // Digital access is now persistent for assigned members and historical stakeholders
    const hasActiveAccess = isSG || isLead || isTeamMember || isInMyGroup;
    ...
    // Visibility: Show practitioner tools (Milestones, Reminders, Pins) if assigned to file or not an executive
    const showPractitionerTools = !isSG || isLead || isTeamMember;
    const canInteract = (hasActiveAccess || wasPreviouslyInvolved) && !isCompleted && !isHistoricalOnly;
    ...
    const handleAddMilestone = async () => {
        if (!newMilestoneTitle.trim() || !file) return;
        setIsSubmitting(true);
        ...
        const updated = [...(file.milestones || []), newMilestone];
        const result = await authUpdateMilestones(file.fileNumber, updated);
        if (result.success) {
    ...
    const handleSendInstruction = async () => {
        ...
        let fromLabel = attorney.fullName;
        if (isSG) fromLabel = `${isActingSG ? 'Acting ' : ''}Solicitor General (${attorney.fullName})`;
        else if (attorney.isActingGroupHead) fromLabel = `Acting Group Head (${attorney.fullName})`;
        
        const result = await authAddInstruction(file.fileNumber, { text: instructionText, from: fromLabel, to: target });
        ...
        <div className="min-h-screen bg-muted/20 pb-20 font-body">
            <header className="bg-background border-b px-4 h-16 flex items-center sticky top-0 z-10 shadow-sm gap-4">
                ...
                <div className="ml-auto flex items-center gap-2">
                    {!isCompleted && showPractitionerTools && !isHistoricalOnly && (<Button variant="ghost" size="sm" className={cn("h-8 gap-2", isPinned ? "text-yellow-600 bg-yellow-50" : "text-muted-foreground")} onClick={handleTogglePin}><Pin className={cn("h-4 w-4", isPinned && "fill-current")} /><span className="hidden sm:inline">{isPinned ? 'Pinned' : 'Pin Case'}</span></Button>)}
                    {isLead && !isCompleted && (
                        <AlertDialog>
            ...
            <main className="container mx-auto p-3 md:p-4 lg:p-6 xl:p-8 space-y-6">
                ...
                <div className="grid gap-6 grid-cols-1 lg:grid-cols-3">
                    <div className="lg:col-span-1 space-y-6">
                        <Card><CardHeader className="pb-3"><CardTitle className="text-xs text-muted-foreground uppercase">Case Summary</CardTitle></CardHeader><CardContent className="space-y-4"><div><Label className="text-[10px] text-muted-foreground uppercase">Lead Assignee</Label><p className="text-sm font-bold text-primary truncate">{file.assignedTo || 'Unassigned'}</p></div><div><Label className="text-[10px] text-muted-foreground uppercase">Current Possession</Label><Badge variant="outline" className="px-2 py-0 h-5 text-[10px] truncate max-w-full bg-blue-50 text-blue-700 border-blue-100">{currentPossessorName}</Badge></div><Separator /><div className="grid grid-cols-2 gap-4"><div><Label className="text-[10px] text-muted-foreground uppercase">Suit Number</Label><p className="text-sm font-medium truncate">{file.suitNumber || 'N/A'}</p></div><div><Label className="text-[10px] text-muted-foreground uppercase">Category</Label><p className="text-sm font-medium capitalize truncate">{file.category}</p></div></div><Separator /><Button className="w-full h-10 gap-2" variant={hasPendingRequest ? "secondary" : "default"} onClick={handleRequestFile} disabled={isSubmitting || hasPendingRequest || isCompleted || isHistoricalOnly}>{isSubmitting ? <Loader2 className="h-4 w-4 animate-spin" /> : <HandIcon className="h-4 w-4" />}{isCompleted ? "Case Completed" : hasPendingRequest ? "Request Pending" : isHistoricalOnly ? "Read Only Access" : "Request Physical File"}</Button></CardContent></Card>
                        ...
                        {showPractitionerTools && (<Card className="border-primary/20"><CardHeader className="pb-3"><CardTitle className="text-sm flex items-center gap-2"><Flag className="h-4 w-4 text-primary" /> Milestones</CardTitle></CardHeader><CardContent className="space-y-4"><div className="space-y-3">{(file.milestones || []).map((milestone) => (<div key={milestone.id} className={cn("flex items-center space-x-3 p-3 rounded-lg border transition-colors", milestone.isCompleted ? "bg-primary/5 border-primary/20" : "bg-muted/30 border-transparent")}><Checkbox id={milestone.id} checked={milestone.isCompleted} onCheckedChange={() => authUpdateMilestones(file.fileNumber, (file.milestones || []).map(m => m.id === milestone.id ? { ...m, isCompleted: !m.isCompleted } : m))} disabled={isCompleted || isHistoricalOnly} /><label htmlFor={milestone.id} className={cn("text-xs font-medium leading-none cursor-pointer", milestone.isCompleted ? "text-primary" : "text-muted-foreground")}>{milestone.title}</label></div>))}</div>{canInteract && (<div className="pt-4 border-t space-y-2"><Label className="text-[10px] uppercase font-bold text-muted-foreground">Add Custom Milestone</Label><div className="flex gap-2"><Input placeholder="e.g. Await Opinion" value={newMilestoneTitle} onChange={(e) => setNewMilestoneTitle(e.target.value)} className="h-8 text-xs" /><Button size="sm" className="h-8 w-8 p-0" onClick={handleAddMilestone} disabled={isSubmitting || !newMilestoneTitle.trim()}>{isSubmitting ? <Loader2 className="h-3 w-3 animate-spin" /> : <Plus className="h-3 w-3" />}</Button></div></div>)}</CardContent></Card>)}
                        {showPractitionerTools && (<Card><CardHeader className="pb-3"><CardTitle className="text-sm flex items-center gap-2"><CalendarIcon className="h-4 w-4 text-primary" /> Reminders</CardTitle></CardHeader><CardContent className="space-y-4">{canInteract && (<div className="space-y-2"><Input placeholder="Task description..." value={reminderText} onChange={(e) => setReminderText(e.target.value)} className="h-8 text-xs" /><div className="flex flex-col sm:flex-row gap-2"><Popover open={isCalendarOpen} onOpenChange={setIsCalendarOpen}><PopoverTrigger asChild><Button variant="outline" className={cn("h-8 text-[10px] flex-1 justify-start font-normal px-2", !selectedDate && "text-muted-foreground")}><CalendarIcon className="mr-1.5 h-3 w-3" />{selectedDate ? format(selectedDate, "MMM d") : "Pick Date"}</Button></PopoverTrigger><PopoverContent className="w-auto p-0" align="start"><Calendar 
                            mode="single" 
    ...
    return (
        <div className="space-y-4">
            <div className="flex justify-between items-center">
                ...
                <div className="flex items-center gap-1.5">{letter.scanUrl && (<Button variant="ghost" size="sm" className="h-7 gap-1.5 text-[10px] text-primary font-bold hover:bg-primary/10" onClick={() => window.open(letter.scanUrl, '_blank')}><Eye className="h-3.5 w-3.5" />View Scan</Button>)}</div>
            </div>
            <div className="grid gap-4 sm:grid-cols-2 mt-4"><div className="space-y-1"><p className="text-xs font-bold text-muted-foreground uppercase">Date</p><p className="text-sm">{toDate(letter.date) ? format(toDate(letter.date)!, 'PPP') : 'N/A'}</p></div><div className="space-y-1"><p className="text-xs font-bold text-muted-foreground uppercase">Doc No.</p><p className="text-sm">{letter.documentNo || 'N/A'}</p></div></div><div className="space-y-1"><p className="text-xs font-bold text-muted-foreground uppercase">Subject</p><p className="text-sm">{letter.subject}</p></div>
        </div>
    );
}
...
function MovementDetails({ movement, index, total, fileNumber, fileSubject, isLatest, onDataChange }: { movement: Movement, index: number, total: number, fileNumber: string, fileSubject: string, isLatest: boolean, onDataChange: () => void }) {
    ...
    return (
        <div className="space-y-4">
            <div className="flex justify-between items-start"><h4 className="text-md font-semibold">Movement #{total - index}</h4>{movement.receivedAt ? (<Badge variant="outline" className="bg-green-50 text-green-700 border-green-200 gap-1.5 py-1"><CheckCircle2 className="h-3.5 w-3.5" /> Confirmed</Badge>) : (!staticOffices.includes(movement.movedTo?.toLowerCase()) ? (<Badge variant="outline" className="bg-yellow-50 text-yellow-700 border-yellow-200 gap-1.5 py-1"><Truck className="h-3.5 w-3.5" /> In Transit</Badge>) : null)}</div>
            ...
            {movement.receivedAt && (<div className="rounded-md bg-muted/50 p-3"><p className="text-[10px] text-muted-foreground uppercase font-bold">Confirmed Receipt By: {movement.receivedBy}</p></div>)}
        </div>
    );
}
