...
export default function PortalFileDetail() {
    ...
    const movements = React.useMemo(() => {
        return [...(file?.movements || [])].sort((a, b) => {
            ...
            // Use ID as final tie-breaker for same-day/same-time movements
    ...
    // SECURITY GATE: Lock digital workspace until physical receipt is confirmed
    const isWorkspaceLocked = isPendingReceipt && !isSG && !isActingSG;

    // Visibility: Show practitioner tools (Milestones, Reminders, Pins) if assigned to file or not an executive
    const showPractitionerTools = !isSG || isLead || isTeamMember || isPossessor;

    const canInteract = hasActiveAccess && !isCompleted && !isHistoricalOnly && !isWorkspaceLocked;
        ...
        <div className="min-h-screen bg-muted/20 pb-20 font-body">
            <header className="bg-background border-b px-4 h-16 flex items-center sticky top-0 z-10 shadow-sm gap-4">
                ...
                <div className="ml-auto flex items-center gap-2">
                    {!isCompleted && showPractitionerTools && !isHistoricalOnly && !isWorkspaceLocked && (<Button variant="ghost" size="sm" className={cn("h-8 gap-2", isPinned ? "text-yellow-600 bg-yellow-50" : "text-muted-foreground")} onClick={handleTogglePin}><Pin className={cn("h-4 w-4", isPinned && "fill-current")} /><span className="hidden sm:inline">{isPinned ? 'Pinned' : 'Pin Case'}</span></Button>)}
                    {isLead && !isCompleted && !isWorkspaceLocked && (
                        <AlertDialog>
            ...
            <main className="container mx-auto p-3 md:p-4 lg:p-6 xl:p-8 space-y-6">
                ...
                {isWorkspaceLocked ? (
                    <div className="flex flex-col items-center justify-center py-20 text-center space-y-6 animate-in fade-in zoom-in-95 duration-500">
                        <div className="bg-muted p-8 rounded-full shadow-inner border-2 border-dashed border-muted-foreground/20">
                            <Lock className="h-16 w-16 text-muted-foreground opacity-30" />
                        </div>
                        <div className="space-y-2">
                            <h2 className="text-2xl font-black text-muted-foreground uppercase tracking-tight">Digital Workspace Locked</h2>
                            <p className="text-muted-foreground max-w-md mx-auto">Access to folios, messaging, and drafts is restricted until physical possession of the folder is confirmed via the arrival banner above.</p>
                        </div>
                    </div>
                ) : (
                    <>
