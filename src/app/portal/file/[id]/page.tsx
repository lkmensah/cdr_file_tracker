...
export default function PortalFileDetail() {
    ...
    // Exceptions: Solicitor General always has oversight
        ...
        <div className="min-h-screen bg-muted/20 pb-20 font-body">
            <header className="bg-background border-b px-4 h-16 flex items-center sticky top-0 z-10 shadow-sm gap-4">
                ...
                <div className="ml-auto flex items-center gap-2">
                    {!isCompleted && !isSG && !isHistoricalOnly && !isWorkspaceLocked && (<Button variant="ghost" size="sm" className={cn("h-8 gap-2", isPinned ? "text-yellow-600 bg-yellow-50" : "text-muted-foreground")} onClick={handleTogglePin}><Pin className={cn("h-4 w-4", isPinned && "fill-current")} /><span className="hidden sm:inline">{isPinned ? 'Pinned' : 'Pin Case'}</span></Button>)}
