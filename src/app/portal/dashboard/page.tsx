    ...
    const notifications = React.useMemo(() => {
        ...
        myRelatedFiles.forEach(file => {
            ...
            if (latest) {
                ...
                if (d && isAfter(d, referencePoint) && !isAfter(d, now)) {
                    if (isSG || latest.movedTo.toLowerCase().trim() === myName) {
                                    ...
                                    <div className="flex items-center gap-2 overflow-hidden">
                                        ...
                                        {isRecentlyUpdated && !isCompleted && type !== 'historical' && (
                                            <div className="flex items-center gap-1 shrink-0">
                                                <div className="h-1.5 w-1.5 rounded-full animate-pulse bg-red-600" />
                                                <span className="text-[8px] font-black uppercase tracking-tight text-red-600">New</span>
                                            </div>
                                        )}
                                    </div>
                    ...
                    <div className="hidden sm:block">
                        <div className="flex items-center gap-2">
                            ...
                            {attorney.isSG ? (
                                ...
                                <Badge variant="secondary" className="bg-muted text-muted-foreground text-[8px] h-4 uppercase font-bold px-1.5 py-0">{attorney.rank || 'Practitioner'}</Badge>
                            )}
                        </div>
                        ...
                        <div className="space-y-12 min-w-0">
                            {isSG ? (
                                ...
                                <div className="space-y-12 min-w-0">
                                    ...
                                    {caseloads.action.length > 0 && <section className="space-y-6"><h3 className="text-xs font-black flex items-center gap-3 text-blue-600 uppercase tracking-[0.2em] border-b pb-4"><UserCheck className="h-5 w-5" /> Shared Files</h3><div className="grid gap-5 grid-cols-1 md:grid-cols-2 lg:grid-cols-3">{caseloads.action.map(file => <FileCard key={file.id} file={file} type="action" />)}</div></section>}
