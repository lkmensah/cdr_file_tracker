...
export default function PortalDashboard() {
    ...
    const caseloads = React.useMemo(() => {
        if (!allFiles || !attorney) return { primary: [], action: [], pinned: [], oversight: [], completed: [], historical: [], all: [] };
        ...
        allFiles.forEach(file => {
            ...
            if (isSG) {
                all.push(file); // Master Registry for SG includes everything (Active + Completed)
        ...
        return { primary, action, pinned, oversight, completed, historical, all };
    ...
    const stagnantOversightFiles = React.useMemo(() => {
        ...
        return sourceList.filter(file => {
            if (file.status === 'Completed') return false; // Stagnation check only for active files
    ...
    const groupWorkloadAnalytics = React.useMemo(() => {
        ...
        [...caseloads.oversight, ...caseloads.completed].forEach(file => {
            const name = file.assignedTo || 'Unassigned';
    ...
    const myRelatedFiles = React.useMemo(() => {
        ...
        [...caseloads.pinned, ...caseloads.primary, ...caseloads.action, ...caseloads.oversight].forEach(f => unique.set(f.id, f));
            ...
            (file.internalInstructions || []).forEach(i => {
                ...
                if (d && isAfter(d, referencePoint) && i.from.toLowerCase().trim() !== myName) {
                    notes.push({
                        ...
                        message: `New message from ${i.from}`,
                        timestamp: d,
                        type: 'communication'
                    });
                }
                ...
                if (d && isAfter(d, referencePoint)) {
                    if (isSG || (latest.movedTo.toLowerCase().trim() === myName && latest.receivedBy?.toLowerCase().trim() !== myName)) {
                        notes.push({
                            ...
                            fileId: file.id,
                            fileNumber: file.fileNumber,
                            message: isSG ? `File moved to ${latest.movedTo}` : `File moved to your desk`,
                            timestamp: d,
                            type: 'movement'
                        });
                    }
    ...
    const availableFiles = React.useMemo(() => {
        ...
        return [...caseloads.pinned, ...caseloads.primary, ...caseloads.action];
    }, [caseloads, allFiles, isSG]);

    const fileOptions = React.useMemo(() => {
        const options = [
            { label: "-- General Activity (No File) --", value: "general" }
        ];
        availableFiles.forEach(file => {
            options.push({
                label: `${file.fileNumber} - ${file.subject}`,
    ...
    const calendarEvents = React.useMemo(() => {
        ...
        const myFiles = allFiles.filter(file => {
            ...
            const movements = [...(file.movements || [])].sort((a,b) => (toDate(b.date)?.getTime() || 0) - (toDate(a.date)?.getTime() || 0));
            const latestMovement = movements[0];
            const isAtMyDesk = latestMovement?.movedTo?.toLowerCase().trim() === attorney.fullName.toLowerCase().trim();
            const fileGroup = file.group?.toLowerCase().trim();
            const isInMyGroup = !!myGroup && fileGroup === myGroup;
            // Calendar shows lead cases, active desk cases, and oversight cases for heads
            return (isLead || isAtMyDesk || (attorney.isGroupHead && isInMyGroup));
        });
        ...
        myFiles.forEach(f => {
            (f.reminders || []).forEach(r => {
                ...
                if (reminderDate) {
                    events.push({ id: r.id, date: reminderDate, text: r.text, type: 'deadline', fileNumber: f.fileNumber, fileId: f.id, isCompleted: r.isCompleted });
                }
            });
            ...
            (f.letters || []).filter(l => l.type === 'Court Process' && l.hearingDate).forEach(l => {
                ...
                if (hearingDate) {
                    events.push({ id: l.id, date: hearingDate, text: `HEARING: ${l.subject}`, type: 'court', fileNumber: f.fileNumber, fileId: f.id });
                }
            });
        });
    ...
    const activeReminders = React.useMemo(() => {
        return calendarEvents.filter(e => e.type === 'deadline');
    }, [calendarEvents]);
    ...
    const handleTogglePin = async (e: React.MouseEvent, fileId: string) => {
        ...
        e.stopPropagation();
        if (attorney) {
            await authTogglePin(fileId, attorney.id);
        }
    }
    ...
    const handleCreateReminder = async () => {
        ...
        if (reminderFileNumber === 'general') {
            result = await authAddGeneralReminder({
                text: reminderText,
    ...
    const handleGenerateStatusReport = () => {
        ...
        try {
            ...
            // 1. Report Header
            doc.setFontSize(18);
            doc.text('Practitioner Status Report', 14, 22);
            doc.setFontSize(10);
            ...
            // 2. Summary Section
            const reportFiles = isSG ? caseloads.all : caseloads.primary;
            ...
            doc.setFontSize(12);
            doc.text('Caseload Summary', 14, 55);
            autoTable(doc, {
            ...
            // 3. Detailed File Data
            doc.setFontSize(12);
            doc.text('Individual Case Status Details', 14, (doc as any).lastAutoTable.finalY + 15);

            const tableData = reportFiles.map(file => {
                // Determine Level (Location)
                const movements = [...(file.movements || [])].sort((a,b) => (toDate(b.date)?.getTime() || 0) - (toDate(a.date)?.getTime() || 0));
                const latest = movements[0];
            ...
            doc.save(`Status_Report_${myName.replace(/\s+/g, '_')}_${format(now, 'yyyy-MM-dd')}.pdf`);
            toast({ title: 'Status Report Generated', description: 'Your assigned files have been exported to PDF.' });
        } catch (error) {
            console.error(error);
    ...
    const FileCard = ({ file, type }: { file: CorrespondenceFile, type: 'pinned' | 'primary' | 'action' | 'oversight' | 'completed' | 'historical' | 'all' }) => {
        const movements = [...(file.movements || [])].sort((a,b) => (toDate(b.date)?.getTime() || 0) - (toDate(a.date)?.getTime() || 0));
        const latestMovement = movements[0];
        ...
        return (
            <Link href={`/portal/file/${file.id}`} className="block relative group min-w-0">
                <Card className={cn(
                    "hover:border-primary/50 transition-colors shadow-sm border-l-4 overflow-hidden",
                        ...
                        <div className="space-y-1 flex-1 min-w-0">
                            <div className="flex items-center gap-2">
                                ...
                                {isCompleted ? (
                                    ...
                                    <Badge variant="secondary" className="text-[9px] uppercase h-4 bg-blue-50 text-blue-700 border-blue-100 shrink-0">Posession</Badge>
                                )}
                            ...
                            <div className="flex items-center gap-2 text-[10px] text-muted-foreground uppercase font-bold tracking-tighter overflow-hidden">
                                <span className={cn("shrink-0", isCompleted ? "text-green-600" : "text-primary")}>
                                    {file.category}
                                </span>
                ...
                {!isCompleted && !isSG && type !== 'historical' && (
                    <Button 
                        ...
                        className={cn(
                            ...
                            file.pinnedBy?.[attorney.id] ? "opacity-100 text-yellow-500" : "opacity-0 group-hover:opacity-100"
                        )}
        ...
        <div className="min-h-screen bg-muted/20 pb-20 font-body">
                ...
                <div className="flex items-center gap-2">
                    ...
                    <div className="hidden sm:block">
                        <div className="flex items-center gap-2">
                            ...
                            {isSG ? (
                                <Badge className="bg-yellow-500 text-white border-yellow-600 text-[8px] h-4 uppercase font-bold px-1.5 py-0">
                                    Solicitor General
                                </Badge>
                            ) : attorney.isGroupHead && (
                                <Badge variant="outline" className="bg-primary/5 text-primary border-primary/20 text-[8px] h-4 uppercase font-bold px-1.5 py-0">
                                    <ShieldCheck className="h-3 w-3 mr-1" /> Group Head
                                </Badge>
                            )}
                        </div>
                ...
                {viewMode === 'monitoring' && (attorney.isGroupHead || isSG) ? (
                    <section className="space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-500 min-w-0">
                        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                            ...
                            <div className="flex items-center gap-3">
                                ...
                                <div className="flex items-center gap-2 p-2 bg-yellow-50 border border-yellow-100 rounded-lg">
                                    ...
                                    <span className="text-[10px] font-bold text-yellow-700 uppercase tracking-wider">Exception Highlight: Stagnant Files (14+ Days)</span>
                                </div>
                            </div>
                        </div>
                                ...
                                <CardHeader className="pb-3 border-b bg-muted/10">
                                    <div className="flex items-center gap-2">
                                        ...
                                        <div>
                                            ...
                                            <CardDescription className="text-[10px] uppercase tracking-tighter font-bold">{isSG ? 'Cross-Group Analysis' : 'Files per Practitioner'}</CardDescription>
                                        </div>
                                    </div>
                                </CardHeader>
                            ...
                            <Card className="shadow-sm border-primary/10">
                                <CardHeader className="pb-3 border-b bg-muted/10">
                                    <div className="flex items-center gap-2">
                                        ...
                                        <div>
                                            <CardTitle className="text-sm">{isSG ? 'Departmental Burden Rank' : 'Practitioner Burden Rank'}</CardTitle>
                                            <CardDescription className="text-[10px] uppercase tracking-tighter font-bold">Comparative active caseload analysis</CardDescription>
                                        </div>
                                    </div>
                                </CardHeader>
                                        ...
                                        <Table>
                                            <TableHeader>
                                                <TableRow className="bg-muted/30">
                                                    ...
                                                    <TableHead className="text-center text-[10px] uppercase font-bold w-[80px]">Active</TableHead>
                                                    <TableHead className="text-right text-[10px] uppercase font-bold w-[100px]">Load Status</TableHead>
                                                </TableRow>
                                            </TableHeader>
                                            <TableBody>
                                                {(isSG ? executiveWorkloadAnalytics : groupWorkloadAnalytics).chartData.map(row => {
                                                    ...
                                                    return (
                                                        <TableRow key={row.name}>
                                                            <TableCell className="text-xs font-bold truncate max-w-[150px]" title={row.name}>{row.name}</TableCell>
                                                            <TableCell className="text-center">
                                                                <span className={cn("text-xs tabular-nums font-bold", isOverloaded ? "text-destructive" : "text-primary")}>
                                                                    {row.active}
                                                                </span>
                                                            </TableCell>
                                                            <TableCell className="text-right">
                                                                {isOverloaded ? (
                                                                    <Badge variant="destructive" className="text-[8px] uppercase tracking-tighter h-4 px-1.5 animate-pulse">High Load</Badge>
                                                                ) : row.active > 0 ? (
                                                                    <Badge variant="secondary" className="text-[8px] uppercase tracking-tighter h-4 px-1.5 bg-green-50 text-green-700 border-green-100">Healthy</Badge>
                                                                ) : (
                                                                    <span className="text-[9px] text-muted-foreground italic">Available</span>
                                                                )}
                                                            </TableCell>
                                                        </TableRow>
                                                    )
                        ...
                        <div className="space-y-4 min-w-0">
                            <h3 className="text-sm font-bold flex items-center gap-2 text-destructive uppercase tracking-widest">
                                <Zap className="h-4 w-4 animate-pulse" /> Stagnant Exception Report
                            </h3>
                            {stagnantOversightFiles.length > 0 ? (
                                        ...
                                        <Table className="min-w-[900px]">
                                            <TableHeader>
                                                <TableRow className="bg-muted/30">
                                                    ...
                                                    <TableHead>Group / Assignee</TableHead>
                                            ...
                                            <TableBody>
                                                {stagnantOversightFiles.map(file => {
                                                    ...
                                                    const currentMilestone = (file.milestones || []).find(m => !m.isCompleted)?.title || "Execution";
                                                    ...
                                                    return (
                                                        <TableRow key={file.id} className="bg-red-50/20">
                                                            <TableCell>
                                                                <span className="font-mono text-xs font-bold text-primary">{file.fileNumber}</span>
                                                            </TableCell>
                                                            <TableCell>
                                                                <div className="flex flex-col max-w-[200px]">
                                                                    <span className="text-xs font-bold text-muted-foreground uppercase tracking-tighter truncate">{file.group || 'No Group'}</span>
                                                                    <span className="text-sm font-medium truncate">{file.assignedTo || 'Unassigned'}</span>
                                                                </div>
                                                            </TableCell>
                                                            <TableCell>
                                                                <div className="flex items-center gap-2 max-w-[150px]">
                                                                    <Flag className="h-3 w-3 text-muted-foreground" />
                                                                    <span className="text-xs font-bold uppercase tracking-tight truncate">{currentMilestone}</span>
                                                                </div>
                                                            </TableCell>
                                                            ...
                                                            <TableCell>
                                                                <Badge variant="destructive" className="bg-red-100 text-red-700 border-red-200 animate-pulse gap-1 px-1.5 py-0.5 whitespace-nowrap">
                                                                    <Zap className="h-3 w-3" /> Stagnant ({daysSince}d)
                                                                </Badge>
                                                            </TableCell>
                                                            <TableCell className="text-right">
                                                                <Button variant="ghost" size="sm" asChild>
                                                                    <Link href={`/portal/file/${file.id}`}>
                                                                        Review
                                                                    </Link>
                                                                </Button>
                                                            </TableCell>
                                ...
                                <div className="flex flex-col items-center justify-center py-24 border border-dashed rounded-xl bg-background shadow-inner text-center space-y-4">
                                    <div className="bg-green-50 p-4 rounded-full border border-green-100">
                                        <ThumbsUp className="h-10 w-10 text-green-600" />
                                    </div>
                                    <div className="space-y-1">
                                        <h4 className="text-xl font-bold text-green-800">All Files Active</h4>
                                        <p className="text-sm text-muted-foreground max-w-sm mx-auto">
                                            Great job! There are no stagnant files within oversight at this time.
                                        </p>
                                    </div>
                                </div>
                            )}
                    ...
                    <>
                        ...
                        {notifications.length > 0 && !searchTerm && (
                            <section className="space-y-3 min-w-0">
                                <h3 className="text-xs font-bold flex items-center gap-2 text-red-600 uppercase tracking-widest">
                                    <Bell className="h-4 w-4 fill-current animate-bounce" /> 
                                    ...
                                    {notifications.map(note => (
                                        <Link key={note.id} href={`/portal/file/${note.fileId}`} className="min-w-0">
                                            <Card className="hover:border-red-200 hover:bg-red-50/30 transition-all border-l-4 border-l-red-500 shadow-sm overflow-hidden">
                                                <CardContent className="p-3 flex items-center justify-between min-w-0">
                                                    <div className="flex items-center gap-3 min-w-0 flex-1">
                                                        <div className="p-2 rounded-full bg-red-100 text-red-600 shrink-0">
                                                            {note.type === 'communication' ? <MessageSquare className="h-4 w-4" /> : 
                                                             note.type === 'folio' ? <FileText className="h-4 w-4" /> :
                                                             note.type === 'draft' ? <Pencil className="h-4 w-4" /> :
                                                             <Truck className="h-4 w-4" />}
                                                        </div>
                                                        <div className="space-y-0.5 min-w-0 flex-1">
                                                            <p className="text-sm font-semibold leading-tight truncate" title={note.message}>{note.message}</p>
                                                            <p className="text-[10px] text-muted-foreground uppercase font-bold tracking-tighter truncate">File {note.fileNumber} • {formatDistanceToNow(note.timestamp)} ago</p>
                                                        </div>
                                                    </div>
                        ...
                        {activeReminders.length > 0 && !searchTerm && (
                            <section className="space-y-3">
                                <h3 className="text-xs font-bold flex items-center gap-2 text-muted-foreground uppercase tracking-widest"><Clock className="h-4 w-4" /> Upcoming Deadlines</h3>
                                <div className="grid gap-2 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3">
                                    {activeReminders.map(reminder => (
                                        <Card key={reminder.id} className={cn("border-l-4 shadow-sm overflow-hidden", isPast(reminder.date) && !isToday(reminder.date) ? "border-l-destructive" : "border-l-primary")}>
                                            <CardContent className="p-3 flex items-start gap-3 min-w-0">
                                                <Button variant="ghost" size="icon" className="h-6 w-6 mt-0.5 shrink-0" onClick={() => handleToggleReminder(reminder.fileNumber, reminder.id, reminder.isGeneral)}><div className="h-4 w-4 rounded border border-muted-foreground" /></Button>
                                                <div className="flex-1 min-w-0">
                                                    <p className="text-xs font-medium truncate" title={reminder.text}>{reminder.text}</p>
                                                    <div className="flex items-center justify-between mt-1 min-w-0">
                                                        <span className="text-[10px] font-mono text-muted-foreground truncate mr-2">{reminder.fileNumber}</span>
                                                        <span className={cn("text-[10px] font-bold shrink-0", isPast(reminder.date) && !isToday(reminder.date) ? "text-destructive" : "text-primary")}>{format(reminder.date, 'MMM d, p')}</span>
                                                    </div>
                                                </div>
                                            </CardContent>
                                        </Card>
                            ...
                            {isSG ? (
                                <section className="space-y-4 min-w-0">
                                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                                        <h3 className="text-xs font-bold flex items-center gap-2 text-yellow-600 uppercase tracking-widest"><LayoutDashboard className="h-4 w-4" /> Master File Registry</h3>
                                        <Badge variant="outline" className="text-[10px] h-5 border-yellow-500 text-yellow-700 bg-yellow-50 uppercase font-bold w-fit">Executive Visibility</Badge>
                                    </div>
                                    ...
                                    {totalPages > 1 && (
                                        <div className="flex flex-col sm:flex-row items-center justify-between border-t pt-6 gap-4">
                                            <p className="text-xs text-muted-foreground">
                                                Showing <span className="font-bold text-foreground">{(currentPage - 1) * PAGE_SIZE + 1}</span> to <span className="font-bold text-foreground">{Math.min(currentPage * PAGE_SIZE, caseloads.all.length)}</span> of <span className="font-bold text-foreground">{caseloads.all.length}</span> records
                                            </p>
                                            <div className="flex items-center gap-2">
                                                <Button 
                                                    variant="outline" 
                                ...
                                <div className="space-y-8 min-w-0">
                                    {caseloads.pinned.length > 0 && (
                                        <section className="space-y-4">
                                            <h3 className="text-xs font-bold flex items-center gap-2 text-yellow-600 uppercase tracking-widest"><Star className="h-4 w-4 fill-current" /> Pinned & Urgent Cases</h3>
                                            <div className="grid gap-4 grid-cols-1">{caseloads.pinned.map(file => <FileCard key={file.id} file={file} type="pinned" />)}</div>
                                        </section>
                                    )}
                                    {caseloads.primary.length > 0 && (
                                        <section className="space-y-4">
                                            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                                                <h3 className="text-xs font-bold flex items-center gap-2 text-muted-foreground uppercase tracking-widest"><Briefcase className="h-4 w-4" /> My Primary Caseload</h3>
                                                <Button 
                                                    variant="outline" 
                                                    size="sm" 
                                                    ...
                                                    disabled={isReporting}
                                                >
                                                    {isReporting ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <FileDown className="h-3.5 w-3.5" />}
                                                    Download My Status Report
                                                </Button>
                                            </div>
                                            <div className="grid gap-4 grid-cols-1">{caseloads.primary.map(file => <FileCard key={file.id} file={file} type="primary" />)}</div>
                                        </section>
                                    )}
                                    {caseloads.action.length > 0 && (
                                        <section className="space-y-4">
                                            <h3 className="text-xs font-bold flex items-center gap-2 text-muted-foreground uppercase tracking-widest"><UserCheck className="h-4 w-4" /> Files on My Desk</h3>
                                            <div className="grid gap-4 grid-cols-1">{caseloads.action.map(file => <FileCard key={file.id} file={file} type="action" />)}</div>
                                        </section>
                                    )}
                                    {caseloads.oversight.length > 0 && (
                                        <section className="space-y-4">
                                            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                                                <h3 className="text-xs font-bold flex items-center gap-2 text-purple-600 uppercase tracking-widest"><Users className="h-4 w-4" /> Group Oversight ({attorney.group})</h3>
                                                <Badge variant="secondary" className="bg-purple-100 text-purple-700 text-[9px] uppercase font-bold w-fit">Group Head View</Badge>
                                            </div>
                                            <div className="grid gap-4 grid-cols-1">{caseloads.oversight.map(file => <FileCard key={file.id} file={file} type="oversight" />)}</div>
                                        </section>
                                    )}
                                    {caseloads.historical.length > 0 && (
                                        <section className="space-y-4 pt-4">
                                            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                                                <h3 className="text-xs font-bold flex items-center gap-2 text-muted-foreground uppercase tracking-widest"><History className="h-4 w-4" /> My Previous Assignments (Historical)</h3>
                                                <Badge variant="secondary" className="bg-muted text-muted-foreground text-[9px] uppercase font-bold w-fit border-none">Read-Only Access</Badge>
                                            </div>
                                            <div className="grid gap-4 grid-cols-1">{caseloads.historical.map(file => <FileCard key={file.id} file={file} type="historical" />)}</div>
                                        </section>
                                    )}
                                </div>
                            )}
                            
                            {(caseloads.completed.length > 0 && !isSG) && (
                                <section className="space-y-4 pt-8 border-t">
                                    <h3 className="text-xs font-bold flex items-center gap-2 text-green-600 uppercase tracking-widest"><Archive className="h-4 w-4" /> Completed & Resolved Cases</h3>
                                    <div className="grid gap-4 grid-cols-1">{caseloads.completed.map(file => <FileCard key={file.id} file={file} type="completed" />)}</div>
                                </section>
                            )}

                            {availableFiles.length === 0 && caseloads.completed.length === 0 && caseloads.historical.length === 0 && (
                                <div className="text-center py-24 border border-dashed rounded-xl bg-background shadow-inner px-4">
                                    <AlertCircle className="h-12 w-12 text-muted-foreground/20 mx-auto mb-4" />
                                    <h4 className="text-xl font-semibold text-muted-foreground">{searchTerm ? 'No results found' : 'Registry is empty'}</h4>
                                    <p className="text-sm text-muted-foreground max-w-xs mx-auto mt-2">Files will appear here once registered or assigned.</p>
                                </div>
                            )}
                        </div>
                    </>
                    ...
                    <Card className="shadow-sm overflow-hidden">
                        <CardHeader className="flex flex-col sm:flex-row items-center justify-between bg-muted/30 border-b gap-4">
                            <div className="text-center sm:text-left">
                                <CardTitle className="text-xl">{format(currentMonth, 'MMMM yyyy')}</CardTitle>
                                <CardDescription>Tracking {calendarEvents.length} active events</CardDescription>
                            </div>
                            <div className="flex items-center gap-2">
                                <Button variant="outline" size="icon" onClick={() => setCurrentMonth(subMonths(currentMonth, 1))}><ChevronLeft className="h-4 w-4" /></Button>
                                <Button variant="outline" size="sm" onClick={() => setCurrentMonth(new Date())}>Today</Button>
                                <Button variant="outline" size="icon" onClick={() => setCurrentMonth(addMonths(currentMonth, 1))}><ChevronRightIcon className="h-4 w-4" /></Button>
                            </div>
                        </CardHeader>
                        <CardContent className="p-0 overflow-x-auto">
                            <div className="min-w-[600px]">
                                <div className="grid grid-cols-7 border-b">{['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'].map(day => (<div key={day} className="p-2 text-center text-[10px] font-bold uppercase tracking-widest text-muted-foreground bg-muted/10">{day}</div>))}</div>
                                <div className="grid grid-cols-7">
                                    {calendarDays.map((day, idx) => {
                                        ...
                                        return (
                                            <div key={day.toString()} className={cn("min-h-[120px] p-2 border-r border-b relative group hover:bg-primary/5 transition-colors min-w-0", !isCurrentMonth && "bg-muted/5 opacity-40", idx % 7 === 6 && "border-r-0")}>
                                                <div className="flex justify-between items-start mb-2">
                                                    <span className={cn("text-xs font-semibold rounded-full h-6 w-6 flex items-center justify-center", isTodayDate && "bg-primary text-primary-foreground")}>{format(day, 'd')}</span>
                                                    {isCurrentMonth && (<Button variant="ghost" size="icon" className="h-6 w-6 opacity-0 group-hover:opacity-100 transition-opacity" onClick={() => handleOpenCreateDialog(day)}><Plus className="h-3 w-3" /></Button>)}
                                                </div>
                                                <div className="space-y-1 overflow-hidden">
                                                    {dayEvents.slice(0, 3).map(event => (
                                                        <div 
                                                            key={event.id} 
                                                            className={cn(
                                                                "text-[9px] px-1.5 py-0.5 rounded border truncate leading-tight font-medium mb-0.5 cursor-pointer hover:brightness-95 transition-all", 
                                                                event.type === 'court' ? "bg-blue-50 text-blue-700 border-blue-100" : 
                                                                isPast(event.date) && !isToday(event.date) ? "bg-red-50 text-red-700 border-red-100" : 
                                                                "bg-green-50 text-green-700 border-green-100"
                                                            )}
                                                            onClick={() => {
                                                                if (event.fileId) {
                                                                    window.location.href = `/portal/file/${event.fileId}`;
                                                                } else {
                                                                    handleToggleReminder('General', event.id, true);
                                                                }
                                                            }}
                                                            title={`${event.fileNumber}: ${event.text}`}
                                                        >
                                                            <span className="font-bold mr-1">{event.fileNumber}:</span>{event.text}
                                                        </div>
                                                    ))}
                                                    {dayEvents.length > 3 && <div className="text-[8px] text-center font-bold text-muted-foreground">+ {dayEvents.length - 3} more</div>}
                                                </div>
                                            </div>
                                        );
                                    })}
            ...
            <Dialog open={isCreateDialogOpen} onOpenChange={setIsCreateDialogOpen}>
                <DialogContent className="w-[95vw] sm:max-w-lg md:max-w-xl p-6 overflow-visible">
                    <DialogHeader><DialogTitle>Set Deadline Reminder</DialogTitle><DialogDescription>Schedule a task for {selectedDateForReminder ? format(selectedDateForReminder, 'PPP') : 'selected date'}.</DialogDescription></DialogHeader>
                    <div className="grid gap-4 py-4 [&_*]:min-w-0 overflow-visible">
                        <div className="space-y-2"><Label>Task Description</Label><Input placeholder="e.g. File Statement of Case" value={reminderText} onChange={(e) => setReminderText(e.target.value)} /></div>
                        <div className="space-y-2 min-w-0 overflow-visible">
                            <Label>Activity / Case File</Label>
                            <div className="min-w-0 w-full overflow-visible">
                                <Combobox 
                                    options={fileOptions}
                                    value={reminderFileNumber}
                                    onChange={reminderFileNumber => setReminderFileNumber(reminderFileNumber)}
                                    placeholder="Select case or general activity..."
                                    searchPlaceholder="Search by file number or subject..."
                                />
                            </div>
                        </div>
                        <div className="space-y-2"><Label>Time</Label><Input type="time" value={reminderTime} onChange={(e) => setReminderTime(e.target.value)} /></div>
                    </div>
                    <DialogFooter className="flex flex-col sm:flex-row gap-3">
                        <Button variant="outline" onClick={() => setIsCreateDialogOpen(false)} className="w-full sm:w-auto order-2 sm:order-1">Cancel</Button>
                        <Button onClick={handleCreateReminder} disabled={isSubmitting || !reminderFileNumber || !reminderText} className="w-full sm:w-auto order-1 sm:order-2">
