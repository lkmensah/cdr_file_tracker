...
import { deleteArchiveRecord } from '@/app/actions';
...
export function ArchivesTable({ records, onEditRecord }: { records: ArchiveRecord[], onEditRecord: (record: ArchiveRecord) => void; }) {
  const { isAdmin } = useProfile();
  const { toast } = useToast();
  const [recordToDelete, setRecordToDelete] = React.useState<ArchiveRecord | null>(null);

  const { exec: authDelete, isLoading: isDeleting } = useAuthAction(deleteArchiveRecord, {
    onSuccess: () => {
        toast({ title: 'Archive record deleted.' });
  ...
  if (records.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center rounded-md border border-dashed py-12 text-center">
        ...
        <h3 className="mt-4 text-lg font-semibold">No Archived Records Found</h3>
        ...
        <Table>
          <TableHeader>
            <TableRow>
              ...
              <TableHead className="hidden md:table-cell">Covering Date (Start)</TableHead>
              <TableHead className="hidden md:table-cell">Covering Date (End)</TableHead>
          ...
          <TableBody>
            {records.map(record => {
              ...
              <TableRow key={record.id}>
                ...
                <TableCell className="hidden md:table-cell">{startDate ? format(startDate, 'PPP') : 'N/A'}</TableCell>
                <TableCell className="hidden md:table-cell">{endDate ? format(endDate, 'PPP') : 'N/A'}</TableCell>
                <TableCell className="hidden sm:table-cell">{record.status}</TableCell>
      ...
      <AlertDialog open={!!recordToDelete} onOpenChange={(open) => !open && setRecordToDelete(null)}>
        <AlertDialogContent>
            <AlertDialogHeader>
                ...
                <AlertDialogDescription>
                    This will permanentely delete this archival record.
                </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
